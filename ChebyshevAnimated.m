...
for i = 1:length(theta)
    ...
    clf
    t = tiledlayout(2,2);
    title(t,'Chebyshev Polynomial T_3 = 4x^3 - 3x')
    ...
    xlabel('x')
    ylabel('T_3(x)')
    plot(x, T3, 'r')     % plot chebychev curve
    lVert = line([cos1(i),cos1(i)], [-1,cos3(i)]);   % vertical (input) line
    lHorz = line([cos1(i),1], [cos3(i),cos3(i)]);    % horizontal (output) line
    lVert.Color = [0.1 0.1 0.1]; lHorz.Color = [0.1 0.1 0.1];
    lVert.LineStyle = '--'; lHorz.LineStyle = '--';
    ...
    ylabel('cos(3\theta)')
    xlabel('\theta')
    xticks([0 pi/3 2*pi/3 pi 4*pi/3 5*pi/3 2*pi])     % label x axis with pi multiples
    xticklabels({'0','\pi/3','2\pi/3','\pi','4\pi/3','5\pi/3','2\pi'})
    plot(theta, cos3)
    plot(currentTheta, cos3(i), 'kx')
    lOut = line([0, currentTheta], [cos3(i), cos3(i)]);
    lOut.Color = [0.1 0.1 0.1]; lOut.LineStyle = '--';
